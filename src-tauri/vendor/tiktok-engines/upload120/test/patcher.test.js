...
function makeMoovWithAudio(chunkOffset) {
  const videoStbl = box('stbl', stts(120, 1), stco(chunkOffset));
  const videoMinf = box('minf', videoStbl);
  const videoMdia = box('mdia', mdhd(120, 120), hdlr('vide'), videoMinf);
  const videoTrak = box('trak', videoMdia);

  const audioStbl = box('stbl', stts(48000, 1024), stco(chunkOffset + 8));
  const audioMinf = box('minf', audioStbl);
  const audioMdia = box('mdia', mdhd(48000, 48000), hdlr('soun'), audioMinf);
  const audioTrak = box('trak', audioMdia);

  return box('moov', mvhd(120, 120), videoTrak, audioTrak);
}
...
function makeSampleMp4WithAudio() {
  const ftyp = box('ftyp', Buffer.from([
    0x69, 0x73, 0x6f, 0x6d,
    0x00, 0x00, 0x00, 0x01,
    0x69, 0x73, 0x6f, 0x6d,
  ...
  const placeholderMoov = makeMoovWithAudio(0);
  const mdat = box('mdat', Buffer.alloc(16, 1));
  const mediaOffset = ftyp.length + placeholderMoov.length + 8;
  const moov = makeMoovWithAudio(mediaOffset);
  return Buffer.concat([ftyp, moov, mdat]);
}
...
function readMdhdTimescales(buf) {
  return findBoxes(buf, 'mdhd').map(header => readU32(buf, header.contentStart + 12));
}
...
test('extension-signal keeps desktop timing normal while adding elst and iTunes metadata', () => {
  ...
  assert.equal(findBoxes(output, 'u120').length, 1);
  assert.equal(hasUtf8(output, 'TikTok Web Signal'), true);
  assert.equal(firstChunkOffset(output), originalOffset + (output.length - source.length));
});
...
test('browser patcher exposes the same public method ids as the Node patcher', () => {
  ...
  assert.deepEqual(
    methodIds,
    ['extension-signal', 'balanced-sync', 'classic-force']
  );
  assert.match(patcher.METHODS[0].description, /Recommended first try/);
});

test('removed weak methods are rejected instead of silently patched', () => {
  const { patchMp4Buffer } = loadWebsitePatcher();
  const source = makeSampleMp4();

  assert.throws(() => patchMp4Buffer(source, { method: 'api-clean', divider: 4 }), /Unknown patch method/);
  assert.throws(() => patchMp4Buffer(source, { method: 'header-lite', divider: 4 }), /Unknown patch method/);
});
...
test('timing methods patch video timing without distorting audio track timing', () => {
  const { patchMp4Buffer } = loadWebsitePatcher();
  const source = makeSampleMp4WithAudio();

  const balanced = outputBuffer(patchMp4Buffer(source, { method: 'balanced-sync', divider: 4 }));
  assert.deepEqual(readMdhdTimescales(balanced), [30, 48000]);
  assert.equal(findBoxes(balanced, 'elst').length, 1);

  const classic = outputBuffer(patchMp4Buffer(source, { method: 'classic-force', divider: 4 }));
  assert.deepEqual(readMdhdTimescales(classic), [30, 48000]);
  assert.equal(findBoxes(classic, 'elst').length, 0);
});
