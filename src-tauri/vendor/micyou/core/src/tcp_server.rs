...
/// Neeko uses the official control message; Android updates its own mute UI.
pub async fn send_mute_command(active: &SharedActiveConnection, muted: bool) -> Result<(), String> {
    let sender = active.lock().await.as_ref().map(|c| c.sender.clone());
    if let Some(sender) = sender {
        sender
            .try_send(MessageWrapper {
                mute: Some(micyou_protocol::micyou::MuteMessage {
                    is_muted: Some(muted),
                }),
                ..Default::default()
            })
            .map_err(|e| format!("No se pudo sincronizar mute con Android: {e}"))?;
    }
    Ok(())
}
    ...
    loop {
        tokio::select! {
            ...
            accept_result = listener.accept() => {
                match accept_result {
                    Ok((socket, addr)) => {
                        // Neeko: never accept public peers, even with forwarded router ports.
                        if !matches!(addr.ip(), std::net::IpAddr::V4(ip) if ip.is_private() || ip.is_loopback() || ip.is_link_local()) {
                            continue;
                        }
...
async fn handle_client(
    ...
    // Neeko: another phone cannot take over an active approved session.
    if active_connection.lock().await.is_some() {
        return Err("A phone is already connected; disconnect it in Neeko first".into());
    }
...
async fn handle_message(
    ...
    if let Some(audio) = msg.audio_packet {
        ...
        // Neeko: TCP/USB deserves the same sample-rate telemetry as UDP.
        if let Some(info) = &audio.audio_packet {
            stats.set_audio_info(info.sample_rate as u32, 0);
        }
    ...
    // Neeko does not load MicYou plugins; ignore their optional wire messages.
    Ok(())
}
