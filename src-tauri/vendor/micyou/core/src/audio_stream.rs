...
pub fn validate_audio_packet(packet: &AudioPacketMessageOrdered) -> bool {
    ...
    // Neeko: reject unknown codecs and non-finite PCM before they enter stateful DSP.
    if !is_opus && audio.codec != micyou_protocol::CODEC_PCM { return false; }
    if !is_opus && audio.audio_format == 4 && audio.buffer.chunks_exact(4)
        .any(|s| !f32::from_le_bytes(s.try_into().unwrap()).is_finite()) { return false; }
