...
impl ServerLifecycleState {
    ...
    pub async fn join_audio_bounded(
        ...
        self.audio_thread = match std::mem::replace(&mut self.audio_thread, AudioThreadState::None)
        {
            AudioThreadState::Owned(thread) => {
                AudioThreadState::Joining(tokio::task::spawn_blocking(move || {
                    thread
                        .join()
                        .map_err(|_| "Audio thread panicked during shutdown".to_string())
                }))
            }
            // Retrying stop must retain an in-progress join, not detach that thread.
            other => other,
        };
...
impl Default for ServerState {
    fn default() -> Self {
        ...
        Self {
            ...
            audio_allowed: Arc::new(std::sync::atomic::AtomicBool::new(false)),
            ptt_blocked: Arc::new(std::sync::atomic::AtomicBool::new(false)),
            local_muted: Arc::new(std::sync::atomic::AtomicBool::new(false)),
            ...
            pcm_sink: Arc::new(std::sync::Mutex::new(None)),
            skip_audio_output: Arc::new(std::sync::atomic::AtomicBool::new(false)),
        }
    }
}

pub struct ServerState {
    /// Neeko: approval and PTT remain local; the adapter synchronizes explicit mute with Android.
    pub audio_allowed: Arc<std::sync::atomic::AtomicBool>,
    pub ptt_blocked: Arc<std::sync::atomic::AtomicBool>,
    pub local_muted: Arc<std::sync::atomic::AtomicBool>,
    ...
    /// Optional PCM sink called from the audio thread after DSP and before
    /// output. The adapter sets this to route audio to the Neeko driver bridge.
    pub pcm_sink: Arc<std::sync::Mutex<Option<Arc<dyn Fn(&[f32]) + Send + Sync>>>>,
    /// When true, the audio thread skips cpal output initialization and push.
    /// Optional sink for processed PCM audio.
    pub skip_audio_output: Arc<std::sync::atomic::AtomicBool>,
