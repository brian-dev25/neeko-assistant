...
// Neeko: bounded, hidden local ADB commands; no remote command strings.
fn command(adb: impl AsRef<std::ffi::OsStr>) -> Command {
...
fn output(cmd: &mut Command) -> std::io::Result<std::process::Output> {
    use std::io::Read;
    ...
    let out = std::thread::spawn(move || {
        let mut b = Vec::new();
        let _ = stdout.take(65536).read_to_end(&mut b);
        b
    });
    let err = std::thread::spawn(move || {
        let mut b = Vec::new();
        let _ = stderr.take(65536).read_to_end(&mut b);
        b
    });
...
pub fn remove_reverse(serial: &str, port: u16) -> Result<(), String> {
    let adb = find_adb().ok_or("ADB no longer available for cleanup")?;
    let result =
        output(command(adb).args(["-s", serial, "reverse", "--remove", &format!("tcp:{port}")]))
...
pub fn list_adb_devices() -> Result<Vec<AdbDevice>, String> {
    ...
    let output = output(command(adb).arg("devices").arg("-l"))
